...
// KruskalWallisByDirectionAndCountry проверяет связь между баллами,
// направлением и желанием учиться в России
func KruskalWallisByDirectionAndCountry(responses []map[string]string) *KruskalWallisResult {
    targetDirections := map[string]bool{
        "DataScience": true,
        "Backend":     true,
        "Embedded":    true,
    }
    ...
    for _, resp := range responses {
        ...
        if !targetDirections[direction] {
            continue
        }

        country := resp["Какую страну для обучения рассматриваете?"]
        ...
        key := direction + "|" + countryKey
        groups[key] = append(groups[key], avgScore)
    }
    ...
    return &KruskalWallisResult{
        GroupName:   "Связь баллов, направления и желания учиться в России",
        HValue:      hValue,
        PValue:      pValue,
...
func normalizeCountry(country string) string {
    switch country {
    case "Россия", "Россия, Беларусь", "Беларусь, Россия":
        return "Россия"
    case "Беларусь":
        return "Беларусь"
    default:
